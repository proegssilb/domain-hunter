...
class TestFlattenedLists:
    ...
    def test_can_flatten_biases_in_lists(self):
        ...
        assert res == {
            ...
            Bias(pattern="zxcv", adjust=2),
            Bias(pattern="brted", adjust=-1),
        }
...
class TestChains:
    @given(
        st.sets(st.text(letters, min_size=1), min_size=1),
        st.sets(st.from_type(Bias)),
        st.integers(1, 10),
    )
...
class TestDataClasses:
    ...
    @given(
        st.lists(st.from_type(ScoredWord), min_size=1),
        st.lists(st.from_type(ScoredWord), min_size=1),
    )
